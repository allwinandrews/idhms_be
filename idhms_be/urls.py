...
urlpatterns = [
    ...
    path("api/admin/analytics/", AdminAnalyticsView.as_view(), name="admin_analytics"),  # NEW: Fetch admin analytics
    # Patient Data Endpoint
    path("api/patient/data/", PatientDataView.as_view(), name="patient_data"),
    # Receptionist-Specific Endpoints
    path("api/receptionist/manage-patients/", ReceptionistManagePatientsView.as_view(), name="receptionist_manage_patients"),
    ...
    path("api/users/", UserListView.as_view(), name="user_list"),  # Supports pagination, search, and sorting
    path("api/users/<int:pk>/", UserDetailView.as_view(), name="user_detail"),  # Retrieve/Update/Delete user
    path("api/users/<int:pk>/roles/", RoleManagementView.as_view(), name="role_management"),  # Assign roles
]
